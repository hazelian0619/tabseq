...
DEFAULT_METHODS = ("tabseq", "catboost", "xgboost", "lightgbm")
...
def _load_baseline_row(metrics_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "target_confidence": _to_float(src.get("confidence")),
        "calibrated_confidence": _to_float(src.get("confidence")),
...
def main() -> None:
    ...
    ap.add_argument("--out-dir", type=str, default=None, help="default: outputs/comparisons/four_methods_<timestamp>")
    args = ap.parse_args()
    ...
    out_dir = Path(args.out_dir).resolve() if args.out_dir else (ROOT / "outputs" / "comparisons" / f"four_methods_{timestamp}")
