...
输出产物：
  - outputs/<dataset>/run_*/checkpoint.pt：模型参数 + 训练配置
  - outputs/<dataset>/run_*/config.json：编码范围/深度/超参（评估必须复用）
  - outputs/<dataset>/run_*/metrics_train.json：训练端最小指标（loss/alpha 统计）
  - outputs/<dataset>/run_*/git.txt：代码版本（复现追踪）
"""
...
def main():
    ...
    dataset = args.dataset or "california_housing"
    ...
    out_dir = args.out_dir or os.path.join("outputs", safe_dataset, f"run_{run_id}")  # 输出目录
    ...
    split = load_dataset_split(dataset, random_state=seed)  # 标准化 + 切分
    X_num = split.X_train  # 数值特征
    y_train = split.y_train  # 连续回归标签
    n_num_features = X_num.shape[1]  # 数值特征维度
    # 若数据集带类别特征，优先使用；否则保持空矩阵占位
    X_cat = split.X_cat_train if split.X_cat_train is not None else np.zeros((len(y_train), 0), dtype=np.int64)
    cat_cardinalities = split.cat_cardinalities or []
    n_cat_features = int(X_cat.shape[1])
    if n_cat_features == 0 and cat_cardinalities:
        raise ValueError("cat_cardinalities provided but X_cat has 0 columns")
    if n_cat_features > 0:
        if not cat_cardinalities:
            raise ValueError("X_cat has columns but cat_cardinalities is empty")
        if len(cat_cardinalities) != n_cat_features:
            raise ValueError(
                f"len(cat_cardinalities)={len(cat_cardinalities)} must match X_cat.shape[1]={n_cat_features}"
            )
