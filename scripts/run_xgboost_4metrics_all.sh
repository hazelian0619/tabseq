...
usage() {
  cat <<'USAGE'
用法:
  scripts/run_xgboost_4metrics_all.sh [options] [-- 透传给 test_xgboost_4metrics.py 的参数]

说明:
  - 默认只跑 diamonds
  - 除脚本自身参数外，其余参数会透传给 scripts/test_xgboost_4metrics.py
...
run_one() {
  ...
  if [[ ${#FORWARD_ARGS[@]} -gt 0 ]]; then
    (cd "$ROOT" && "$PYTHON_BIN" scripts/test_xgboost_4metrics.py --dataset "$ds" "${FORWARD_ARGS[@]}")
  else
    (cd "$ROOT" && "$PYTHON_BIN" scripts/test_xgboost_4metrics.py --dataset "$ds")
  fi
}
...
echo "[RUN] xgboost four-metrics batch"
