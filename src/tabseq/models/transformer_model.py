...
class DacaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        gate: torch.Tensor,  # (B, T, D) 每个样本每一步的门控向量
    ) -> torch.Tensor:
...
class TransformerTabSeqModel(nn.Module):
    ...
    def __init__(
        ...
        depth: int,  # 决策序列长度
        n_bins: int,  # 叶子桶数，必须等于 2^depth（二叉树深度）
        ...
        expected_bins = 1 << self.depth
        if self.n_bins != expected_bins:
            raise ValueError(
                f"n_bins={n_bins} must equal 2^depth={expected_bins}. "
                f"Current: depth={depth}, expected_bins={expected_bins}"
            )
        ...
        # 因果 mask：防止 t 看到 t+1 之后的 token（bool 更稳）
        tgt_mask = torch.triu(torch.ones((self.depth, self.depth), dtype=torch.bool), diagonal=1)
        ...
        # DACA 门控：G_t = sigmoid(MLP(E_pos(t)))（仅随 depth 变化，对齐 PDF）
        # 保留 ctx_gate_proj 仅用于旧 checkpoint 兼容（forward 中不使用）
    ...
    def forward(self, batch: dict, return_context: bool = False) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        if seq_len != self.depth:  # 防御：必须与 depth 对齐
            raise ValueError(f"dec_input length {seq_len} must equal depth {self.depth}")
        if dec_input.numel() > 0:
            if int(dec_input.min().item()) < 0 or int(dec_input.max().item()) > 2:
                raise ValueError("dec_input values must be in {0,1,2}")
        ...
        # 3) DACA gate: G_t = sigmoid(MLP(E_pos(t)))（仅由 depth 决定）
        # NOTE: paper mentions early/late depth intuition (e.g., t<=3 / t>=8) but does not hard-code thresholds.
        pos_gate = self.daca_gate_mlp(pos_emb)  # (T, d_model)
        gate = torch.sigmoid(pos_gate).unsqueeze(0).expand(tok.shape[0], -1, -1)  # (B, T, d_model)

        # 4) 解码：自回归 self-attention + cross-attention(ctx_tokens)
        tgt_mask = self._tgt_mask[:seq_len, :seq_len].to(device=tok.device)
    ...
    def compute_loss(
        self,
        logits: torch.Tensor,           # (B, depth, n_bins)
        y_mht: torch.Tensor,            # (B, depth, n_bins)
        ctx_tokens: torch.Tensor,       # (B, n_ctx, d_model) 从 forward 传入
        temperature: float = 1.0
    ) -> torch.Tensor:
        """ACM 损失计算（高效版）"""
        B, depth, n_bins = logits.shape
        
        # 1. 概率
        probs = torch.sigmoid(logits / temperature)
        
        # 2. BCE
        bce = -(y_mht * torch.log(probs + 1e-8) + (1 - y_mht) * torch.log(1 - probs + 1e-8))
        
        # 3. alpha_depth
        depth_indices = torch.arange(depth, dtype=torch.float32, device=logits.device)
        alpha_depth = 1.0 - 0.5 * (depth_indices / (depth - 1))
        alpha_depth = alpha_depth.view(1, depth, 1)
        
        # 4. alpha_instance
        alpha_instance = self.compute_alpha_instance(ctx_tokens).view(B, 1, 1)
        
        # 5. 组合
        alpha = alpha_depth * alpha_instance
        masked_loss = alpha * bce.mean(dim=2, keepdim=True)
